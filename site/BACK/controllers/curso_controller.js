...
exports.crearCurso = (req,res)=>{
    ...
    if(nombre && categoriaCurso && descripcion && diasDeCurso && horariosDeCurso && vacantes && precio){
        ...
        res.status(400).json({mensaje: "Faltan datos para poder crear el curso"})
    }
...
exports.obtenerCurso = (req,res)=>{
    ...
    if(id){
        ...
        res.status(400).json({mensaje: "Ingrese un id"})
    }
...
exports.obtenerTodosLosCursos = (req,res)=>{
    ...
    if(cursos){
        ...
        res.status(400).json({mensaje: "No existen cursos en la base de datos"})
    }
...
exports.eliminarCurso = (req,res)=>{
    ...
    if(id){
        Curso.deleteOne( {_id: { $eq: id}}, (error, response)=>{
            if (error) return res.status(400).json({ error });
            if (response) return res.status(200).json({mensaje: "El curso se ha eliminado correctamente"});
        } )
